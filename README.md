...
Pi получает скилл и настоящее расширение: команда `/pohuy` включает режим `full`,
`/pohuy lite|full|ultra` меняет калибр, а `/pohuy normal` возвращает обычный режим.
Расширение само загружает SKILL.md, словарь, сцены и онтологию при старте и добавляет
готовый комплект правил в системный промпт. Агенту не нужно вызывать `read` или вручную
искать файлы скилла.

Выбор сохраняется глобально в `~/.pi/agent/settings.json`, поэтому действует после
перезапуска Pi и в других проектах. Команда не принимает лишние аргументы - состояние
остаётся как было.
...
# ответ остаётся технически точным, но в режиме ultra

> /pohuy normal
Pohuy disabled. Normal mode restored.
```

После выбора расширение добавляет в `~/.pi/agent/settings.json` только свой ключ:
...
Значение `normal` выключает стиль и тоже сохраняется. Остальные настройки Pi остаются
нетронутыми. Расширение добавляет стиль только к естественному тексту ответов. Вызовы
инструментов, код, команды, идентификаторы, ошибки, структурированный вывод, коммиты, PR
и документация остаются без мата. Security и необратимые операции - тоже без шуток.
